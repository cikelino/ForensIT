...
const FROM = process.env.EMAIL_FROM; // es. "Migrazioni <onboarding@tuodominio.it>"
...
async function send(payload, label) {
  ...
  if (error) {
    ...
    console.log(`Resend OK (${label}), id:`, data?.id);
  }
}
...
// (può essere il collega, se ha proposto l'admin, o viceversa)
